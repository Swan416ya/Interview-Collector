...
关于 stem（题干）——必须遵守：
- 面经多为面试者事后回忆，正文往往碎片化、口语化、顺序跳跃。**禁止大段照抄原文节选**；不要把「我记得」「当时」「还问了」「面试官说」等叙述性废话写进 stem。
- 你要根据碎片信息，把每个考点**还原成面试官当面会问的、完整规范的一句或一小段话**：书面语、术语统一、主谓宾补全，读起来像正式面试题，而不是笔记摘录。
- 若几条碎片其实在考同一考点，可**合并**为一条 stem；若是不同考点则拆成多条，每条 stem 单独成题、可独立作答。
- 优先用**疑问句**收束（以「？」结尾）；用「请简述/说明/介绍/分析」等指令式也可以，但必须完整、正式。
- 技术要点须与原文一致，不要编造原文不存在的考点；原文含糊时可做合理归纳，并贴近上下文。
...
5) 除 stem 内的必要题干外，输出必须精简，禁止长段解释。
6) 输出格式严格为：
{{
  "questions": [
    {{
