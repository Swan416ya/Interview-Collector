...
- [x] 答题记录页：
  - [x] 全局记录分页
  - [x] 日期筛选（`shanghai_date`）
  - [x] 详情展开（完整题干、用户答案、AI解析）
...
- [x] 空态/加载态/错误态统一
- [x] 大文本渲染（解析与参考答案，支持换行）
- [x] Web 与 Android 调试地址文档复核
- [x] 冒烟回归（5 大页面各一条完整链路）
...
- 已完成：`Home` / `Questions` / `Practice` / `PracticeHistory` / `AnswerRecords` 功能对齐
- 已完成：Flutter Web 与 Android 本地调试方式文档化
- 已完成：基础回归（`flutter analyze`、`flutter test`）通过
